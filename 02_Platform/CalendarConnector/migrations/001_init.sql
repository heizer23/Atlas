...
-- Token fields are stored plaintext for this slice — known security gap,
-- see implementation_notes.md and architecture.json risks.
...
-- One row per calendar_connection row (UNIQUE constraint on connection_id).
...
-- One row per in-flight connect flow; deleted on callback consumption (one-time use).
