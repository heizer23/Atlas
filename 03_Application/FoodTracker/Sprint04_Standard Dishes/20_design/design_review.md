...
1. **Migration file path is inconsistent across artifacts**
   - Severity: Critical
   - Location: `architecture.json → persistence.schema_artifact` vs. `architecture.json → persistence.sprint04_note` and `architecture.json → deferrals.application_implementer[0]` and `scaffolding.json → files[0].path`
...
2. **`row_actions` includes `'copy'` but the Sprint 04 UI removes the Copy menu item without product resolution**
   - Severity: Major
   - Location: `scaffolding.json → files[1] (entries.py) → public_objects[1] (list_entries) → methods[0].purpose` ("Row_actions remain ['delete', 'copy', 'edit']") and `architecture.json → open_questions[0]`
...
1. **Resolve the migration path to a single canonical reference**
   - Location: `architecture.json → persistence.schema_artifact`
...
2. **Discharge the Copy action question before scaffold handoff**
   - Location: `architecture.json → open_questions[0]` and `scaffolding.json → files[1] → public_objects[1].purpose`
...
3. **Correct the invalid SQL fragment in `internal_flow` step 2**
   - Location: `architecture.json → internal_flow[1] (fetch_standards_page) → description`
...
1. **`StandardsPagePayload`, `StandardDish`, and `TodayInstance` types are private to `StandardsPage.tsx`**
   - Location: `scaffolding.json → files[5] (StandardsPage.tsx) → private_objects`
...
2. **`_serialise_standard_log_result` duplicates `_serialise_entry_detail` output shape**
   - Location: `scaffolding.json → files[1] (standards.py) → private_objects[0]`
   - Observation: The scaffold correctly prohibits `standards.py` from importing `_serialise_entry_detail` from `entries.py`, requiring a parallel private serialiser. The risk of divergence on future EntryDetail field additions is acknowledged in `architecture.json → risks[4]`. The scaffold is consistent with its own rule but the parallel implementation is a known maintenance liability.
...
1. **UI_Data_Contract — `row_actions` declared by producer not honoured by consumer**
   ...
   - Location: `scaffolding.json → files[1] (entries.py) → public_objects[1] (list_entries).purpose` declares `row_actions=['delete', 'copy', 'edit']`; `scaffolding.json → files[4] (EntriesPage.tsx) → private_objects[1] (ThreeDotsMenu)` renders only Standard/Remove Standard/Delete.
...
1. **Whether `'edit'` is a valid Sprint 04 row action**
   - Location: `scaffolding.json → files[1] (entries.py) → public_objects[1] (list_entries).purpose`
...
2. **`DELETE /api/food/entries/{id}` can silently delete a standard row from the Entries page**
   - Location: `architecture.json → risks[2]`
...
1. Update `architecture.json → persistence.schema_artifact` from `03_Application/FoodTracker/Sprint04_Standard Dishes/20_design/migration_004.sql` to `03_Application/FoodTracker/migrations/003_add_standard_fields.sql` to match all other references.
2. Obtain product confirmation on the Copy action fate (retain as flat button outside ThreeDotsMenu, or remove from Entries page in Sprint 04), then update `scaffolding.json → files[1] (entries.py) → list_entries.purpose → row_actions` and the ThreeDotsMenu specification to be consistent.
3. Remove the invalid `ORDER BY SUM(logged_at) DESC` clause fragment from `architecture.json → internal_flow[1] (fetch_standards_page) → description`, retaining only `MAX(logged_at) DESC`.
